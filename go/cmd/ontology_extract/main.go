...
func main() {
	...
	schemaPath := flag.String("schema", "", "Path to ontology schema JSON (required)")
	parquetPath := flag.String("parquet", "", "Path to UDML Parquet storage (required)")
	outputPath := flag.String("output", "", "Output path for extracted entities/relationships JSON (optional, stdout if not provided)")
	docID := flag.String("doc-id", "", "Filter by document ID (optional)")
	...
	// Extract entities - choose distributed or single-worker mode
	...
	// Always use distributed coordinator (works with any number of workers, including 1)
	log.Printf("🤖 Extracting entities and relationships with %d workers...\n", *workers)
	jobControl := ontology.NewMemoryExtractionJobControl()
	coordinator := ontology.NewExtractionCoordinator(
		&schema,
		ontologyElements,
		targetDocID,
		elements[0].SourceName,
		jobControl,
		storage,
		contentResolver,
	)
