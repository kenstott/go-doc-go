...
This document describes the JSON Schema definitions for UDML (Universal Document Model Language) validation.

**Schema Files Location**: All JSON schema files (`.json`) are located in the `/schemas` directory at the project root.

## Schema Files

UDML provides **three JSON schemas** for different format variants:

### 1. Persistence Format Schema (`udml-1.0.schema.json`) - **UDML-S**

**Purpose**: Validates the flat array format used for storage and SQL queries

**Format Name**: UDML-S (Storage/Persistence)
...
### 2. Hierarchical Format Schema (`udml-1.0-hierarchical.schema.json`) - **UDML-H**

**Purpose**: Validates the nested format used for processing and JSONPath queries

**Format Name**: UDML-H (Hierarchical/Nested)
...
### 3. Graph Format Schema (`udml-1.0-graph.schema.json`) - **UDML-G**

**Purpose**: Validates the graph format (UDML-G) used for graph database exports and network analysis

**Format Name**: UDML-G (Graph/Property Graph)
