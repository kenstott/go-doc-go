...
class ElementBase(BaseModel):
    ...
    # Ordering fields for document reconstruction
    element_order: Optional[int] = None  # Position within parent container (0-based)
    document_position: Optional[int] = None  # Global position in document (0-based)
...
def flatten_hierarchy(elements: List[ElementHierarchical], parent_path: str = "") -> List[ElementFlat]:
    ...
    # Sort the flat list by document_position if available, otherwise by path
    if flat_list and hasattr(flat_list[0], 'document_position') and flat_list[0].document_position is not None:
        # Sort by document position to preserve original document order
        return sorted(flat_list, key=lambda x: x.document_position if x.document_position is not None else float('inf'))
    else:
        # Fallback to path-based sorting
