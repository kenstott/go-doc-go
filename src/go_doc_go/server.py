...
def search_endpoint():
    """
    Unified search endpoint supporting multiple backends.

    Request body:
    {
        "query": "search text",
        "search_service": "parquet_lake",  // Search service to use (REQUIRED: 'parquet' or 'parquet_lake')
        "limit": 10,
        "min_score": 0.0,
        "filter_criteria": {},
        ...
        // Enhanced features for parquet backend:
        "filters": {
            "regex_pattern": "(sales|revenue|decline)",
            "element_types": ["paragraph", "+comment", "-table"], 
            "cosine_threshold": 0.3
        },
        "context": {
            "parents": 2,
            "siblings": 2,
            "semantic_relationships": 3,
    ...
    try:
        ...
        if not search_service:
            # Get available search services from registry
            available_services = []
            analytics_backends = _config.list_analytics_backends()
            for name, backend in analytics_backends.items():
                if backend.get('enabled', True):
                    available_services.append(name)
            
            raise BadRequest(f"'search_service' parameter is required. Available services: {', '.join(available_services)}")
        
        # Get the search service configuration from registry
        backend_config = _config.get_analytics_backend(search_service)
        
        # Check if service exists
        if backend_config is None:
            # Service not found in registry - list available services
            available_services = []
            analytics_backends = _config.list_analytics_backends()
            for name, backend in analytics_backends.items():
                if backend.get('enabled', True):
            ...
            raise BadRequest(f"Invalid search_service '{search_service}'. Available services: {', '.join(available_services)}")
        
        # Check if the backend is enabled
        if not backend_config.get('enabled', True):
            raise BadRequest(f"Search service '{search_service}' is currently disabled")
        
        # Check if backend type is parquet (only supported type for now)
        if backend_config.get('type') != 'parquet':
            raise BadRequest(f"Search service '{search_service}' uses unsupported backend type '{backend_config.get('type')}'. Only 'parquet' type is currently supported")
        
        # Use comprehensive parquet search
        filters = data.get('filters', {})
        context_config = data.get('context', {})
        reconstruction_config = data.get('reconstruction', {})
        
        # Override limit if specified in filters
        if 'limit' not in filters:
            filters['limit'] = min(data.get('limit', CONFIG['DEFAULT_RESULTS']), CONFIG['MAX_RESULTS'])
        
        # Add cosine threshold from min_score if not specified
        if 'cosine_threshold' not in filters and data.get('min_score'):
            filters['cosine_threshold'] = data.get('min_score', 0.0)
        
        search_results = _search_parquet_with_context(
            query=query_text,
    ...
    except (BadRequest, NotFound, Unauthorized, Forbidden) as e:
...
@app.route('/api/search/parquet', methods=['POST'])
def search_parquet_data():
    """
    Advanced search against parquet data lake with comprehensive context.
    
    Request body:
    {
        "query": "sales downturn",
        "backend": "parquet_lake",
        "filters": {
            "regex_pattern": "revenue.*decline",
            "element_types": ["paragraph", "div", "table"],
    ...
    try:
        ...
        backend = data.get('backend', 'parquet_lake')
        filters = data.get('filters', {})
        context_config = data.get('context', {})
        reconstruction_config = data.get('reconstruction', {})
        
        # Get backend configuration
        backend_config = _config.get_analytics_backend(backend)
        if not backend_config:
            raise BadRequest(f"Backend '{backend}' not found")
        
        if backend_config.get('type') != 'parquet':
            raise BadRequest(f"Backend '{backend}' is not a parquet backend")
        
        # Perform parquet search
        search_results = _search_parquet_with_context(
            query=query,
            backend_config=backend_config,
            filters=filters,
        ...
        logger.warning(f"Bad request for parquet search: {str(e)}")
        return jsonify({'error': 'Bad Request', 'message': str(e)}), 400
    except Exception as e:
        logger.error(f"Parquet search error: {str(e)}")
