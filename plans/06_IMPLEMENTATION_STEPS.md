...
## Step 6: Update Domain Catalogs & Implement Global Catalog Merging

### Part A: Update Catalog Files
...
### Part B: Implement Global Catalog Auto-Merge in Builder

#### Files

- `go/internal/udml/ontology/builder.go` - Add global catalog merging logic

#### Requirement

**ALL schemas generated by ontology interview MUST automatically include global entity types and relationship rules verbatim.**

The global domain catalog (located in `examples/ontologies/global/`) defines 37 reusable entity types across the 6 W's framework. These types must be merged into every generated schema to provide baseline entity extraction capabilities.

#### Implementation

Add new method `mergeGlobalCatalog()` that:

1. **Load global catalog files** from `examples/ontologies/global/`:
   - `global_who.yaml` (person, organization + 10 subtypes)
   - `global_what.yaml` (document, product, concept + 6 subtypes)
   - `global_where.yaml` (location + 5 subtypes)
   - `global_when.yaml` (date, event + 4 subtypes)
   - `global_why.yaml` (reason, goal + 3 subtypes)
   - `global_how.yaml` (method, process + 3 subtypes)

2. **Merge entity types**:
   - Prepend global entity mappings to schema's `element_entity_mappings`
   - Keep LLM-generated domain-specific entities after global ones
   - No deduplication - global + domain-specific can coexist

3. **Merge relationship rules**:
   - Append global relationship rules to schema's `entity_relationship_rules`
   - Global relationships provide baseline patterns between global entities

4. **Add global domain** to domains list:
   - Create `Domain{Name: "global", Description: "Universal entity types", Owner: "System"}`
   - Add to beginning of `schema.Domains` slice

#### Code Location

Update `generateDraftSchema()` in `builder.go`:

```go
func (b *OntologyBuilder) generateDraftSchema(...) (*OntologySchema, int, int, error) {
    ...
    // NEW: Merge global catalog into schema
...
func (b *OntologyBuilder) mergeGlobalCatalog(schema *OntologySchema) error {
    // 1. Load global catalog files
    globalCatalogs, err := catalogs.LoadFromDirectory("examples/ontologies/global")
    if err != nil {
        return fmt.Errorf("failed to load global catalogs: %w", err)
    }

    // 2. Merge entity types (prepend global entities)
    globalEntities := []ElementEntityMappingConfig{}
    for _, catalog := range globalCatalogs {
        globalEntities = append(globalEntities, catalog.EntityTypes...)
    }
    schema.ElementEntityMappings = append(globalEntities, schema.ElementEntityMappings...)

    // 3. Merge relationship rules (append global relationships)
    globalRels := []EntityRelationshipRule{}
    for _, catalog := range globalCatalogs {
        globalRels = append(globalRels, catalog.Relationships...)
    }
    ...
    globalDomain := Domain{
        Name:        "global",
        Description: "Universal entity types and baseline patterns",
        Owner:       "System",
    }
...
- Generated schemas contain all 37 global entity types
- Global entities appear BEFORE domain-specific entities
- Global relationships are included
- Global domain is listed first in domains array
...
**Part A (Catalog Updates)**: ~200 lines across 36 files (~5-6 lines per file average)

**Part B (Builder Changes)**: ~50 lines in `builder.go`:
- +30 lines: `mergeGlobalCatalog()` method
- +5 lines: Call to `mergeGlobalCatalog()` in `generateDraftSchema()`
- +15 lines: Error handling and logging
